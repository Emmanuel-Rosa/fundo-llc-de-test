...
DuckDB because it's just a file — no server to run — and because it's strict about types,
which caught two real problems that a more forgiving database would have hidden.
...
*succeeds* it stops nothing and the command never comes back to your prompt. I found that by
running it.

### Then one more command — this is the failing-check demo
...
Every command writes what it printed to `docs/<command>.log` on your machine, so a run leaves
evidence behind instead of scrolling away. Those files are git-ignored, so following these
...
- `break` puts back every value it changed, then **re-reads them to confirm**. It doesn't
  trust a rowcount. And the values it writes back are the ones it read a moment earlier —
  nothing is hardcoded, so it can't corrupt the data if the seed ever changes.
- Merging duplicates never edits or deletes a customer row. It writes a separate mapping
  table. Undoing a merge is a one-row change, not a restore from backup.
...
less**; the quieter one moves 0.45%. And run 4 is the important one: running it again when
nothing has changed reads nothing at all. That's what makes it safe to put on a schedule.

About that "data moved" number: it's the size of the *values* transferred. It leaves out
protocol overhead and encryption, so it's a floor, not a network bill. It's a fair basis for
comparing a delta against a full copy because both are measured the same way.
...
One table is most of the rows and almost none of the daily change. That's the whole reason to
pick a strategy per table instead of using one everywhere.
...
second, about 3.5%. Nothing would have told you.
...
transaction commits — and those are different moments. So a row can be invisible to your read
while a *higher* ID has already committed. I set that up with two connections: one inserts ID
3 and holds the transaction open, another inserts ID 4 and commits. A reader then sees IDs 1,
2 and 4, and records 4 as the high-water mark. When ID 3 finally commits, it's already below
the mark and will never be read again — 50,000 cents, gone quietly.
...
| `FAIL` | the warehouse doesn't match the source — the copy is broken | 1 |
...
in transit would hide a real problem behind a warehouse that looks fine.

With only pass and fail, every clean run would fail on those orphans. The usual fix is to
hardcode "expect 20 orphans" — which is a number that goes stale the first time the data
changes. An earlier version of this code did exactly that and printed a discrepancy that
wasn't real.
...
skip them **based on their type, not by name** — which is how I found the second one, since
nothing had it written down anywhere. The choice is between saying "I skipped these" and
comparing them with a fudge factor and calling it a pass. I'd rather say it.
...
gap is a strong hint rather than proof. Those are real limits, and a check
that hides its limits gets trusted further than it deserves.
...
the minimum and the maximum are all *identical* — so every check passes, and two transactions
are still attributed to the wrong rows. This limitation was written down in two files before
anyone ran it, and something written down but never run is a claim, not a finding. Catching it
needs comparing every row one by one, which the checks only do for a table that already looks
wrong — doing it always would cost as much as the full copy this replaces.
...
address identifies a mailbox, not a person. There are four cases in the test data to make that
happen — a family address, a shared office inbox, roommates, and a father and son with the
same name.

First names are left out, because nicknames aren't evidence — Robert and Bobby are the same
person and the fields don't say so. Surnames are in, and I tested taking them out too: it
correctly catches one person who changed their name after marriage, and wrongly merges two
strangers who happen to share a birth date and the last four digits of their SSN.

**Fake-looking values are caught by their shape, not by a list.** `0000`, `1234`, `5555555555`
— anything that's all the same digit or a straight run. A list of known-bad values is a list
someone has to keep adding to, and when it misses one the failure is silent: an unrecognised
...
The review note includes each person's full repayment-account fingerprint, because the
question a human actually needs to answer is whether these two repay from the *same* account.
...
that, so it isn't shortened.

**2. Test data is excluded, not merged — and yes, the obvious pattern catches real people.**
...
domain *and* an automation-shaped name, not just one.

**3. Bad phone numbers and emails: found, counted, and left alone.** There are four
categories, not two — valid, malformed, placeholder, and missing — and keeping them apart
matters because `placeholder` is the dangerous one: `5555555555` is a perfectly well-formed
phone number that identifies nobody. Out of 4,970 customers there are **6 malformed phone
numbers and 1 placeholder email**.
...
customer. Refusing the field costs only the matches that field would have made — and those are
exactly the matches you shouldn't make based on a broken value.
...
keeps a broken phone number when a merged-away duplicate had a good one. The alternative
builds a person who never existed out of pieces of several records, and then nobody can reach
them at all.
...
database, **no customer has two default cards** — that rule is intact. After merging,
**27 customers have two**, and every single one of those is a case where two people were merged
into one. All of it is caused by the merge; none of it was already there. Two people each
picked their own default card, they turn out to be the same person, and now nothing can say
...
would mean the pipeline making a call about someone's money that nobody asked it to make. Of
everything in here, this is the one I'd put in front of the business rather than decide myself.
...
**That precision of 1.000 is not as impressive as it looks, and I'd rather say so than let it
...
Recall — 0.770, the lower number — is the more useful one.
...
are cases the resolver deliberately refused. A miss it *chose* to make is a different thing
from one it failed to find.

### Merges get withdrawn, not patched

The map is rebuilt from scratch on every run. It's never edited in place. That sounds like a
detail and it's the reason the withdrawal above works: day one gives a merged group a fourth
member, day two gives that member a funded advance, and the next run *un-merges* what the
previous run merged — without anyone editing any data. If you only resolved once at the end,
you'd never see it. That's why the demo resolves four times instead of once.
...
| | change | precision | recall | what it cost |
|---|---|---|---|---|
| | as shipped | 1.000 | 0.770 | 47 proposed, 47 correct, 0 wrong, 14 missed |
| CF-1 | trust email as proof | 0.961 | 0.803 | 2 wrong merges, 2 correct ones found |
| CF-2 | stop filtering placeholder values | 1.000 | 0.770 | nothing changes |
| CF-3 | add first name to the match | 1.000 | 0.770 | nothing changes |
| CF-4 | drop surname from the match | 0.980 | 0.787 | 1 wrong merge, 1 correct one found |
| CF-5 | exclude test accounts *after* merging | 1.000 | 0.770 | same merges, **14 more accounts scored** |
| CF-6 | use the naive `%test%` filter | 1.000 | 0.770 | same merges, **7 real people dropped, 7 fakes kept** |

Four of those rows look identical to the shipped one, and only two of them really are.

**CF-2 and CF-3 genuinely change nothing here.** So their value rests on reasoning rather than
...
what it changed. Now a change in who got scored is reported as a change.
...
no test framework — the whole dependency list is three packages, and adding a fourth for this
wouldn't earn its place.
...
Test files are found by pattern, not listed anywhere, so adding one can't be silently skipped —
`test_connect` was picked up automatically the moment it existed. Each file runs in its own
process, because two of them install a fake database driver and a test suite whose result
depends on what ran first is worse than no test suite.

`test` runs first inside `docker compose up` and stops before building anything if something
is red. None of it needs a database, and the build takes minutes — failing the fast part first
is the entire point of having a fast part.
...
you hand it. Those tests write down the engine rules that actually bit.
...
error handling and is wrong. Worse, it *worked* several times before it failed, because it
depended on how fast the database started. `test_connect` is there so it can't come back.
...
Dockerfile             Python 3.12 — both database drivers ship prebuilt for it, so no compiler
pipeline/
  tables.yml           which tables, which columns, which strategy. Anything not listed is not copied
  config.py            everything adjustable. A number buried in code disagrees with the write-up later
...
sql/source/            schema, then seed data, then turn on change tracking — always that order
...
flaws in it, each one labelled, and each one causes something visible in the output rather than
just having a comment claiming it would.
...
- **It asks for 4 GB of memory, and that's measured rather than picked.** At 2 GB — which is
  ...
  It's not the container running out of memory; Docker had 16 GB free. A limit that depends on
  the day isn't a limit anyone can reproduce.
